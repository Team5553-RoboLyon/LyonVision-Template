...
void Vision::run() {
  ...
  /* Port 0 is default camera, but needs a webcam with a mac adress, usually an
  external camera, on either port 0 or 1
  And it's also to note, USB cameras for tinkerboards and Pi's start from 4
  onwards, because weird reasons.*/
  cs::CvSink sink{"USB"};
  sink.SetSource(cam);
  ...
  // This lets us see the camera output on the robot dashboard. We give it a
  // name, and a width and height.
  cs::CvSource output =
      frc::CameraServer::GetInstance()->PutVideo("USB Camera", video_mode.width, video_mode.height);

  cv::Mat imgOriginal{video_mode.height, video_mode.width, CV_8UC3};

  // Changes the exposure for detecting retroReflective Tape and gives minimal
  // error (requires ring light, And only works once deployed to a Pi/Tinker)
  if (usingTapeTrack)
    ;
  ...
  while (true) {
    if (sink.GrabFrame(imgOriginal) != 0) {
      ...
      cv::Mat imgTracking;
      cvtColor(imgOriginal, imgTracking,
               COLOR_BGR2HSV);  // Convert the captugreen frame from BGR to HSV
